...
import babyHeartFrequencyStore, { BabyHeartFrequency, BabyHeartFrequencyStore } from '../store/BabyHeartFrequency/babyHeartFrequencyStore';
 ...
 */
const BabyGraph: React.FC<BabyGraphProps> = observer(({ }) => {
    ...
    const babyHeartFrequencyList = babyHeartFrequencyStore.getBabyHeartFrequencyList(patientDataStore.selectedPartogrammeId).slice();
    ...
    const data = sortedData.map((point: BabyHeartFrequency['Row']) => {
        return { x: getCurrentRelativeX(point.created_at), y: point.babyFc };
    });
